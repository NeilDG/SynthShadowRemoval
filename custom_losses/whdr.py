...
def load_image(filename, is_srgb=True):
    ...
    # print("Load sRGB filename: ", filename)
    if filename == None:
...
def whdr_final(ref, jud):
    reflectance = load_image(filename=ref, is_srgb=False)
    judgements = json.load(open(jud))

    whdr = compute_whdr(reflectance, judgements, 0.1)
