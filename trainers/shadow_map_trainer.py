...
class ShadowMapTrainer:
    ...
    def __init__(self, gpu_device,  opts):
        ...
        if(net_config == 1):
            self.G_A = cycle_gan.Generator(input_nc=7, output_nc=1, n_residual_blocks=num_blocks).to(self.gpu_device)
        elif(net_config == 2):
            self.G_A = unet_gan.UnetGenerator(input_nc=7, output_nc=1, num_downs=num_blocks).to(self.gpu_device)
        elif (net_config == 3):
            self.G_A = cycle_gan.Generator(input_nc=7, output_nc=1, n_residual_blocks=num_blocks, has_dropout=False).to(self.gpu_device)
        elif (net_config == 4):
            self.G_A = cycle_gan.GeneratorV2(input_nc=7, output_nc=1, n_residual_blocks=num_blocks, has_dropout=False, multiply=True).to(self.gpu_device)
        else:
            self.G_A = cycle_gan.GeneratorV2(input_nc=7, output_nc=1, n_residual_blocks=num_blocks, has_dropout=False, multiply=False).to(self.gpu_device)
...
class ShadowMapTrainerBasic:
    ...
    def __init__(self, gpu_device, opts):
        ...
        if (net_config == 1):
            self.G_A = cycle_gan.Generator(input_nc=3, output_nc=1, n_residual_blocks=num_blocks).to(self.gpu_device)
        elif (net_config == 2):
