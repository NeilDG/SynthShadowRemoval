...
class NetworkCreator():
    ...
    def initialize_shadow_network(self, net_config, num_blocks, input_nc):
        if (net_config == 1):
            G_Z = cycle_gan.Generator(input_nc=input_nc, output_nc=4, n_residual_blocks=num_blocks).to(self.gpu_device)
        elif (net_config == 2):
            G_Z = unet_gan.UnetGenerator(input_nc=input_nc, output_nc=4, num_downs=num_blocks).to(self.gpu_device)
        elif (net_config == 3):
            G_Z = cycle_gan.Generator(input_nc=input_nc, output_nc=4, n_residual_blocks=num_blocks, has_dropout=False, use_cbam=True).to(self.gpu_device)
        elif (net_config == 4):
            params = {'dim': 64,  # number of filters in the bottommost layer
                      'mlp_dim': 256,  # number of filters in MLP
                      'style_dim': 8,  # length of style code
                      ...
                      'n_res': num_blocks,  # number of residual blocks in content encoder/decoder
                      'pad_type': 'reflect'}
            G_Z = usi3d_gan.AdaINGen(input_dim=input_nc, output_dim=4, params=params).to(self.gpu_device)
        else:
            G_Z = cycle_gan.GeneratorV2(input_nc=input_nc, output_nc=4, n_residual_blocks=num_blocks, has_dropout=False, multiply=False).to(self.gpu_device)

        D_Z = cycle_gan.Discriminator(input_nc=4).to(self.gpu_device)  # use CycleGAN's discriminator
