...
class RenderMapsTrainer:
    ...
    def train(self, a_tensor, b_tensor, train_mask):
        with amp.autocast():
            ...
            likeness_loss = self.l1_loss(clean_like, b_tensor) * self.l1_weight
            lpip_loss = self.lpip_loss(clean_like, b_tensor) * self.lpip_weight
            ssim_loss = self.ssim_loss(clean_like, b_tensor) * self.ssim_weight
