...
class ShadowRelightTrainerPrior:
    ...
    def prepare_input(self, a_tensor, shadow_prior, light_angle):
        # light_angle = self.normalize(light_angle)
        # light_angle_tensor = torch.unsqueeze(torch.full_like(a_tensor[:, 0, :, :], light_angle), 1)
        # print("Shapes: ", np.shape(a_tensor), np.shape(light_angle))
        concat_input = torch.cat([a_tensor, light_angle], 1)
    ...
    def train(self, a_tensor, b_tensor, shadow_prior, light_angle):
        with amp.autocast():
            a2b = self.G_A(self.prepare_input(a_tensor, shadow_prior, light_angle))
    ...
    def test(self, a_tensor, shadow_prior, light_angle):
        with torch.no_grad():
            a2b = self.G_A(self.prepare_input(a_tensor, shadow_prior, light_angle))
        return a2b
