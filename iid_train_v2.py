    ...
    for mode in (["train_shadow"]):
        ...
        for epoch in range(start_epoch, general_config[mode]["max_epochs"]):
            for i, (train_data, test_data, rw_data) in enumerate(zip(train_loader, test_loader, itertools.cycle(rw_loader))):
                ...
                rgb_ws_tensor, rgb_ns_tensor, albedo_tensor, shading_tensor, shadow_matte_tensor = iid_op(rgb_ws_tensor, rgb_ns_tensor, albedo_tensor)

                input_map = {"rgb": rgb_ws_tensor, "rgb_ns" : rgb_ns_tensor, "albedo": albedo_tensor, "unlit": unlit_tensor, "shading" : shading_tensor, "shadow_matte" : shadow_matte_tensor}
